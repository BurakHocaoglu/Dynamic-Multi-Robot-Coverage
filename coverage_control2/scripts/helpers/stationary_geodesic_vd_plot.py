...
def plot_region_and_agents(ax, p_i, p_j, Q, holes=[]):
	...
	# ax.add_artist(plt.Circle((xmin - 1., ymin - 1.), 0.2))
	# ax.quiver(xmin - 1., ymin - 1., np.cos(0.), np.sin(0.), 
	# 	scale=2., scale_units="inches", lw=0.5)
	# ax.quiver(xmin - 1., ymin - 1., np.cos(np.pi / 2), np.sin(np.pi / 2), 
	# 	scale=2., scale_units="inches", lw=0.5)
	# ax.text(xmin + 3.3, ymin - 1.3, "+x", color=(0., 0., 0.))
	# ax.text(xmin - 1.5, ymin + 3.5, "+y", color=(0., 0., 0.))
