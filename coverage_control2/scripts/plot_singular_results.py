	...
	for count in [8, 12, 16]:
		...
		for file in files:
			...
			with open("{}/{}/{}".format(exp_data_path, count, file), 'r') as D:
				...
				w_ax.set_ylabel("Grid Workload")
				w_ax.set_xlim(0., nSteps)
				for aid, e_data in exp_data.items():
					w_ax.plot(x_axis, e_data["workload"], alpha=0.5)
				plt.savefig("{}/{}/{}_workload.png".format(exp_data_path, count, file[:-5]), bbox_inches="tight")
				...
				g_ax.set_ylabel("Max. Geod. Distance")
				g_ax.set_xlim(0., nSteps)
				for aid, e_data in exp_data.items():
					g_ax.plot(x_axis, e_data["max_geod"], alpha=0.5)
				plt.savefig("{}/{}/{}_max_geod.png".format(exp_data_path, count, file[:-5]), bbox_inches="tight")


				a_figure = plt.figure(num=fig_num + 2)
				a_ax = a_figure.add_subplot(1, 1, 1)
				# a_ax.set_aspect("equal")
				a_ax.set_xlabel("Time")
				a_ax.set_ylabel("Actual Workload")
				a_ax.set_xlim(0., nSteps)
				for aid, e_data in exp_data.items():
					a_ax.plot(x_axis, e_data["actual_work"], alpha=0.5)
				plt.savefig("{}/{}/{}_actual_work.png".format(exp_data_path, count, file[:-5]), bbox_inches="tight")
