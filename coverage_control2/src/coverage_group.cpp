...
std::unordered_map<std::uint8_t, Agent> coverage_group;
std::unordered_map<std::uint8_t, std::unique_ptr<std::thread>> runners;

void run(Agent& agent, ros::Rate& rate) {
...
int main(int argc, char **argv) {
	uint8_t gid = std::stoi(argv[1]);
	agent_id_start = std::stoi(argv[2]);
	agent_group_size = std::stoi(argv[3]);

	std::string node_name = "Group" + std::string(argv[1]);

	ros::init(argc, argv, node_name.c_str());
	ros::NodeHandle node;

	for (uint8_t i = agent_id_start; i < agent_group_size; i++) {
		std::string agent_name = "Agent" + std::to_string(i);
		coverage_group.emplace(i, Agent(node, agent_name, i));
	}
	...
	if (node.getParam("/motion_params", motion_params)) {
		...
		for (auto& member : coverage_group) {
			member.second.set_motion_params(m_params);
		}
	...
	if (node.getParam("/sensing_params", sensing_params)) {
		...
		for (auto& member : coverage_group) {
			member.second.set_sensing_params(s_params);
		}
	...
	if (node.getParam("/behaviours", behaviours)) {
		...
		for (auto& member : coverage_group) {
			member.second.set_behaviour_settings(b_settings);
		}
	} else {
	...
	for (auto& member : coverage_group) {
		member.second.set_task_region_from_raw(exp_region, exp_obstacles);
	}
	...
	while (ros::ok()) {
		...
		for (auto& member : coverage_group) {
			ready = ready && member.second.ready();
		}
	...
	ROS_INFO("%s has warmed up. Spawning...", node_name.c_str());
	ros::Duration(1.).sleep();

	for (auto& member : coverage_group) {
		runners.emplace(member.second.get_id(), new std::thread(&Agent::));
	}
